...
1. **Call `mirror_status`** exactly once. If `staleMinutes > 90`, warn the user that data may be stale and ask whether to proceed. If `staleMinutes <= 90`, note the freshness in one sentence at the top of the answer ("Data through <lastSyncedAt>") and continue.
...
### Recipe 1 — "Make me a 20-minute podcast script with this week's actionable insights"

20 minutes of audio is ~2,800–3,400 words spoken. Target ~3,000 words, 5–7 segments.

1. `mirror_status` — check freshness.
2. Compute `since = today - 7 days` as ISO date.
3. Gather candidates by **importance + diversity**:
   - `search_items(since=<7d>, limit=50)` sorted by recency (all categories).
   - `aggregate_items(group_by='source', since=<7d>, limit=10)` → note top-5 active sources.
   - `aggregate_items(group_by='category', since=<7d>, limit=9)` → check for dead categories to skip.
   - For each of the top 3–4 themes you're hearing, run `semantic_search_items(query=<theme>, limit=5)` to round out coverage.
4. Cluster items into 5–7 thematic segments. For each segment: pick 2–4 strongest items, `get_item` on them to pull direct quotes/details.
5. Structure the script:
   - 30s intro ("Week of <dates>, here's what I'd keep an eye on")
   - 5–7 segments × 2–3 min each; each segment = theme + specific items + concrete action the listener could take
   - 30s outro + preview of something to watch next week
6. Write to a file (e.g. `out/podcast-script-YYYY-MM-DD.md`) rather than dumping inline.
7. End with a small table of citations (title, source, URL) at the bottom so the user can verify.
