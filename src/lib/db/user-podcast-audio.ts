...
export async function listUserPodcastAudio(
  ...
  if (driver === "postgres") {
    ...
    return (result.rows as unknown as Array<Record<string, unknown>>).map((r) => ({
      ...
      segmentAudio: r.segment_audio ? (JSON.parse(r.segment_audio as string) as PodcastAudioRecord["segmentAudio"]) : undefined,
...
export async function deleteUserPodcastAudio(
  ...
  if (driver === "postgres") {
    ...
    const result = await client.run(
      `DELETE FROM user_podcast_audio WHERE user_id = $1 AND audio_id = $2`,
      [uid, audioId]
    );
  ...
  const sqlite = getSqlite();
  const result = sqlite.prepare(`DELETE FROM user_podcast_audio WHERE user_id = ? AND audio_id = ?`).run(uid, audioId);
  return result.changes > 0;
}
