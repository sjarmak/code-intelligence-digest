...
function shouldExcludeUrl(url: string): boolean {
  ...
  if (urlLower.includes("newsletter.") && urlLower.includes(".com")) {
    ...
    if (
      isNewsletterDomain &&
      (urlLower.includes("/subscribe") || urlLower.includes("subscribe?"))
    ) {
      return true;
    }
  }
  ...
  for (const domain of newsletterDomains) {
    if (urlLower.includes(domain)) {
      ...
      if (
        pathMatch &&
        homepagePaths.includes("/" + pathMatch[1].toLowerCase())
      ) {
...
function extractArticlesFromHtml(html: string): Array<{
  ...
  const sectionPatterns = [
    {
      pattern: /(?:big tech[^<]*&amp;?[^<]*startups?|big tech[^<]*startups?)/i,
      name: "Big Tech & Startups",
      category: "ai_news" as Category,
    },
    {
      pattern: /programming[^<]*(?:&amp;|and)[^<]*(?:design|data science)/i,
      name: "Programming, Design & Data Science",
      category: "newsletters" as Category,
    },
    {
      pattern: /programming[^<]*design/i,
      name: "Programming & Design",
      category: "newsletters" as Category,
    },
    {
      pattern: /science[^<]*(?:&amp;|and)[^<]*futuristic[^<]*technology/i,
      name: "Science & Futuristic Technology",
      category: "newsletters" as Category,
    },
    {
      pattern: /miscellaneous/i,
      name: "Miscellaneous",
      category: "newsletters" as Category,
    },
  ];
  ...
  function findUrlNearby(
    startIndex: number,
    maxDistance: number = 500,
  ): string | null {
    const searchText = cleanHtml.substring(
      startIndex,
      startIndex + maxDistance,
    );
  ...
  while ((match = htmlLinkPattern.exec(cleanHtml)) !== null) {
    ...
    const strongMatch = rawTitleHtml.match(
      /<strong[^>]*>([\s\S]*?)<\/strong>/i,
    );
    if (strongMatch) {
    ...
    if (!title || title.length < 5) {
      ...
      const beforeLink = cleanHtml.substring(
        Math.max(0, match.index! - 200),
        match.index!,
      );
      const textBefore = beforeLink
        .replace(/<[^>]*>/g, " ")
        .replace(/\s+/g, " ")
        .trim();
      ...
      if (trackingMatch) {
        ...
        if (isValidAbsoluteUrl(decoded)) {
          ...
          if (decoded.includes("localhost") || decoded.includes("127.0.0.1")) {
            logger.debug(
              `Filtered out decoded TLDR URL with localhost: ${decoded}`,
            );
            continue;
          }
          ...
          if (shouldExcludeUrl(decoded)) {
            logger.debug(
              `Filtered out decoded TLDR homepage/subscription URL: ${decoded}`,
            );
            continue;
          }
          url = decoded;
    ...
    if (rawUrl.includes("substack.com/redirect/2/")) {
      const base64Match = rawUrl.match(
        /substack\.com\/redirect\/2\/([A-Za-z0-9_-]+)/,
      );
      if (base64Match) {
        try {
          ...
          if (
            ...
            if (shouldExcludeUrl(decodedUrl) || isSubscribeUrl) {
              logger.debug(
                `Filtered out decoded Substack subscription URL: ${decodedUrl}`,
              );
              continue;
            }
            url = decodedUrl;
  ...
  while ((match = urlOnlyRegex.exec(cleanHtml)) !== null) {
    ...
    if (title && url && !seen.has(url)) {
      ...
      if (isSubstackDomain && !normalizedUrl.includes("/p/")) {
        logger.debug(
          `Pattern 3: Filtered out Substack non-article URL (missing /p/): ${normalizedUrl}`,
        );
        continue; // Skip this URL - it's not an article
      }
  ...
  const readOnlinePattern =
    /(?:read\s+online|read\s+article)[^<]*<a[^>]*href=["']([^"']+)["'][^>]*>/gi;
  while ((match = readOnlinePattern.exec(cleanHtml)) !== null) {
    ...
    if (
      ...
      if (!shouldExcludeUrl(normalizedUrl) && !shouldExcludeTitle(title)) {
        ...
        logger.debug(
          `Extracted "Read Online" link: ${title} -> ${normalizedUrl}`,
        );
      }
    }
  }
  ...
  const knownTerms = [
    {
      term: "Delve",
      patterns: [/delve[^<]*shipmas[^<]*day[^<]*\d+/i, /delve[^<]*sponsor/i],
    },
    { term: "Vibium", patterns: [/vibium[^<]*github/i, /vibium[^<]*repo/i] },
    {
      term: "Package Manager",
      patterns: [/package manager[^<]*git[^<]*database/i, /git as a database/i],
    },
    {
      term: "Groq",
      patterns: [
        /groq[^<]*ai[^<]*technology/i,
        /nvidia[^<]*groq/i,
        /groq[^<]*chip/i,
      ],
    },
    {
      term: "NotebookLM",
      patterns: [
        /transform sources.*structured.*data tables.*notebooklm/i,
        /notebooklm.*data tables/i,
      ],
    },
    {
      term: "Codex vs Claude",
      patterns: [/codex.*vs.*claude code/i, /codex vs claude.*today/i],
    },
    {
      term: "Memory: How Agents Learn",
      patterns: [/memory.*how agents learn/i],
    },
  ...
  for (const { term, patterns } of knownTerms) {
    for (const pattern of patterns) {
      ...
      for (const match of matches) {
        ...
        const linkMatches = context.match(
          /<a[^>]*href=["']([^"']+)["'][^>]*>/gi,
        );
        if (linkMatches) {
            ...
            if (urlMatch) {
              ...
              if (url.includes("/CL0/")) {
                ...
                if (trackingMatch) {
                  const decoded = trackingMatch[1]
                    ...
                    .replace(/%3F/g, "?");
                  if (
                    isValidAbsoluteUrl(decoded) &&
                    !decoded.includes("localhost") &&
                    !shouldExcludeUrl(decoded)
                  ) {
                    url = decoded;
                  } else {
                    continue;
              ...
              if (
                ...
                if (!shouldExcludeTitle(title)) {
                  ...
                  seen.add(url);
                  logger.debug(
                    `Extracted article by known term "${term}": ${title} -> ${url}`,
                  );
                }
              }
            }
  ...
  while ((match = headerPattern.exec(cleanHtml)) !== null) {
    ...
    const normalizedUrl = nearbyUrl
      ? nearbyUrl.replace(/&amp;/g, "&").trim()
      : null;
    ...
    if (
      ...
      if (
        ...
        logger.debug(
          `Pattern 6: Filtered out Substack non-article URL (missing /p/): ${normalizedUrl}`,
        );
        continue; // Skip this URL - it's not an article
      }
...
function createArticleItem(
  ...
  if (!finalUrl || !isValidAbsoluteUrl(finalUrl)) {
    logger.debug(
      `Article "${article.title}" has empty or invalid URL: "${finalUrl}"`,
    );
  ...
  if (
    ...
    logger.debug(
      `Filtered out localhost URL for article "${article.title}": ${finalUrl}`,
    );
    finalUrl = ""; // Clear it so we try to find a better URL
  }
  ...
  if (
    !finalUrl ||
    !isValidAbsoluteUrl(finalUrl) ||
    finalUrl.includes("inoreader.com")
  ) {
    ...
    if (urlMatch) {
      ...
      if (
        ...
        logger.info(
          `Extracted fallback URL for article "${article.title}": ${finalUrl}`,
        );
      }
    }
  }
  ...
  if (
    ...
    if (
      ...
      logger.warn(
        `No valid URL found for article "${article.title}", using base item URL: ${baseItem.url}`,
      );
      finalUrl = baseItem.url;
    } else {
      logger.warn(
        `No valid URL found for article "${article.title}", skipping (base URL is invalid or excluded: ${baseItem.url})`,
      );
  ...
  if (!finalUrl || !isValidAbsoluteUrl(finalUrl)) {
    logger.warn(
      `Article "${article.title}" has no valid URL after all attempts, will be skipped`,
    );
  ...
  return {
    ...
    finalScore: baseItem.finalScore * 0.9,
...
export function decomposeNewsletterItem(item: RankedItem): RankedItem[] {
  ...
  if (articles.length === 0) {
    logger.warn(
      `No articles extracted from newsletter: "${item.title}" (${htmlContent.length} chars of content)`,
    );
    ...
    if (shouldExcludeUrl(item.url) || shouldExcludeTitle(item.title)) {
      logger.info(
        `Excluding newsletter item as subscription/promotional content: "${item.title}"`,
      );
  ...
  if (articles.length === 1) {
    ...
    if (
      !article.url ||
      !isValidAbsoluteUrl(article.url) ||
      shouldExcludeUrl(article.url)
    ) {
      logger.debug(
        `Filtering out single article with invalid/excluded URL: "${article.title}" (URL: "${article.url}")`,
      );
    ...
    logger.info(
      `Single article found in ${item.sourceTitle}: "${article.title}"`,
    );
    return [
      {
        ...item,
  ...
  logger.info(
    `Decomposing ${item.sourceTitle} into ${articles.length} articles`,
  );
  logger.info(
    `[DECOMPOSE_DEBUG] Article URLs extracted: ${articles
      .slice(0, 3)
      .map((a) => a.url)
      .join(" | ")}`,
  );
  ...
  const decomposed = articles
    ...
    .filter((decomposedItem) => {
      ...
      if (!decomposedItem.url || !isValidAbsoluteUrl(decomposedItem.url)) {
        logger.debug(
          `Filtering out decomposed item with invalid URL: "${decomposedItem.title}" (URL: "${decomposedItem.url}")`,
        );
        return false;
      }
      ...
      if (shouldExcludeUrl(decomposedItem.url)) {
        logger.debug(
          `Filtering out decomposed item with excluded URL: "${decomposedItem.title}" (URL: "${decomposedItem.url}")`,
        );
        return false;
      }
      return true;
  ...
  logger.info(
    `[DECOMPOSE_DEBUG] Decomposed item URLs: ${decomposed
      .slice(0, 3)
      .map((d) => d.url)
      .join(" | ")}`,
  );
...
export function decomposeFeedItem(item: FeedItem): FeedItem[] {
  ...
  if (!htmlContent || htmlContent.length < 100) {
    ...
    logger.warn(
      `Newsletter item "${item.title}" has no content to decompose (${htmlContent.length} chars)`,
    );
    return [item];
  }
  ...
  for (const [idx, article] of articles.entries()) {
    ...
    // Store section information in summary for later categorization
    const sectionInfo = article.section ? ` [SECTION:${article.section}]` : "";
    const summaryWithSection =
      (article.snippet || item.summary || "") + sectionInfo;
  ...
  logger.info(
    `[DECOMPOSE_DEBUG] Decomposed ${decomposed.length} valid articles from ${articles.length} extracted`,
  );
