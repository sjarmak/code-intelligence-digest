 ...
 * Uses Tavily when available, with Parallel API fallback.
 */
  ...
  if (tavilyApiKey?.trim()) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logger.info("Tavily web search completed", {
          query: query.slice(0, 60),
          resultCount: results.length,
        });
        return results;
      ...
      const text = await response.text();
      logger.warn("Tavily search failed, falling back to Parallel", {
        status: response.status,
        body: text.slice(0, 200),
      });
      return await searchParallel(query, options);
    } catch (error) {
      logger.error("Tavily web search error, falling back to Parallel", { error, query: query.slice(0, 60) });
      return await searchParallel(query, options);
    }
  }

  logger.debug("TAVILY_API_KEY not set, using Parallel fallback if available");
  return await searchParallel(query, options);
}
